...
#賃金の種類を選べるようにセレクトボックス化
option_wage = st.selectbox(
    '賃金の種類',
    (wage_list))
#セレクトボックスで選択した集計年のデータで抽出したdfを作成
df_mean_categ = df_jp_category[(df_jp_category['集計年']==option_year)]

#賃金種類をセレクトできる仕様とするため、どの賃金種類を選択しても値の範囲は０〜最大値に指定する必要がある
#このため、max_xという変数を作成。最後の+50はグラフを見やすくするための工夫
max_x = df_mean_categ[option_wage].max()+50

#plotlyで可視化（産業分類毎、年齢別で棒グラフ化）
fig = px.bar(df_mean_categ,#データフレーム
             x = option_wage,#x軸にはセレクトボックスを指定
             y = '産業大分類名',#y軸のラベル名
             color = '産業大分類名',##棒グラフの色を何を軸につける。今回は産業大分類名
             animation_frame = '年齢',#年齢の推移に応じてアニメーションを設定
             range_x = [0, max_x],#xの範囲を指定
             orientation = 'h',#
             width=800,#横幅を指定
             height=500#縦幅を指定
)
